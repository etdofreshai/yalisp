...
Updated: 2026-09-01 M1 complete, active M2 reader/printer, macro, and cap slices green. Scale: 0
...
| --- | ---: | --- | --- |
| Semantic correctness | 2 | M0 is 301/301; M1 adds 15 reviewed cases/17 events with no divergence; M2 adds seeded reader/printer and pinned macro properties plus exact input/depth/work/expansion caps | Broader malformed source generation and nested quasiquote/splice cases |
...
| Code size / instructions | 2 | Current seed is 10,393 bytes with 4,237 static instruction lines; layer bytes/hashes and one 43-byte AOT artifact are recorded | Automated per-stage size/static count and representative dynamic counts |
...
| Macro expansion determinism | Independent named outer expansion; 8 authored cases hash `34214d55...` across 4 fresh and 16 warmed corpus runs; produced user mutation is not evaluated; step 1,025 fails explicitly | M2 nested quasiquote/splice semantics |
| IR/decompile invariants | No IR | M4 |
| Property/fuzz and malformed caps | Seeded acyclic-data property, two malformed diagnostics, binary shrink 511/512, exact 32,768/32,769 work boundary, 130,048-byte host cap, and minimal self-expanding macro witness | M2 broader malformed syntax corpus |
...
and input/depth/work/expansion caps have exact passing/failing witnesses. The
earliest remaining gap is a broader malformed-source corpus, beginning with
nested quasiquote/unquote-splicing and malformed dotted/closing forms. M2 is
active.
