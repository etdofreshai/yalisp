...
Updated: 2026-09-01 M1 complete, active M2 reader/printer slice green. Scale: 0
...
| --- | ---: | --- | --- |
| Semantic correctness | 2 | M0 is 301/301; M1 adds 15 reviewed cases/17 events with no divergence; M2 adds 256 seeded data round trips plus 23 fixed reader/printer cases and two stable malformed diagnostics | Independent macro-expansion hashes, shrink evidence, and bounded malformed/depth properties |
...
| Code size / instructions | 2 | Current seed is 9,935 bytes with 4,066 static instruction lines; layer bytes/hashes and one 43-byte AOT artifact are recorded | Automated per-stage size/static count and representative dynamic counts |
...
expected or cross-stage divergence. M2's first reader/printer slice is green at
seed `0x59414c49`. The earliest remaining gap is an independently observable,
deterministically hashed macro expansion across fresh and long-lived sessions;
persisted shrinking and general malformed reader depth/work caps follow. M2 is
active.
