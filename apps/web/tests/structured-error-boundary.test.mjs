...
test("intentional seed diagnostics expose stable category records", async () => {
  for (const fixture of [
    { category: "reader", code: 2, diagnostic: "read error", method: "read", source: ")" },
    { category: "arity", code: 3, diagnostic: "unquote expected", source: "(quasiquote (unquote))" },
    { category: "type", code: 4, diagnostic: "string expected", source: "(string.length 1)" },
    { category: "apply", code: 5, diagnostic: "cannot apply", source: "(1 2)" },
    {
      category: "arithmetic", code: 6, diagnostic: "value exceeds fixnum range",
      source: "(begin (define m3.word (bytes.alloc 4)) (u8! m3.word 3 64) (i32@ m3.word 0))",
    },
    { category: "bounds", code: 7, diagnostic: "byte index out of range", source: "(u8@ (bytes.alloc 1) 1)" },
    { category: "resource-exhausted", code: 8, diagnostic: "memory limit reached", source: "(heap.reserve 1073741824)" },
  ]) {
    const session = await seedHarness.createSeedSession({ boot: false });
...
test("host-ingest protocol failures have a seed-owned category", async () => {
  ...
  assert.throws(() => instance.exports.asset_commit(), (error) => {
    const classified = seedHarness.classifySeedTrap(
      error, instance.exports.error_kind(), "asset ingest protocol",
    );
